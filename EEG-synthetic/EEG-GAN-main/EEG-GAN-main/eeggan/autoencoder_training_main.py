...
def main(args=None):
    ...
    manual_kw_conditions = []
    filtered_args = []
    
    for arg in raw_args:
        if arg.startswith('kw_conditions='):
            # 1. Estraiamo il valore
            val = arg.split('=', 1)[1]
            # 2. Pulizia: rimuoviamo eventuali parentesi quadre o apici inseriti per errore
            val = val.replace('[','').replace(']','').replace("'", "").replace('"', "")
            # 3. Trasformiamo la stringa "Cond1,Cond2" in una lista reale ["Cond1", "Cond2"]
            if ',' in val:
                manual_kw_conditions = [c.strip() for c in val.split(',')]
            else:
                manual_kw_conditions = [val.strip()]
        else:
            filtered_args.append(arg)
    
    # Passiamo al parser solo gli argomenti "sicuri"
    default_args = system_inputs.parse_arguments(filtered_args, file='autoencoder_training_main.py')
    
    # Re-inseriamo la lista pulita. 
    # Se non è stato passato nulla, usiamo il default del parser (solitamente [''])
    if manual_kw_conditions:
        default_args['kw_conditions'] = manual_kw_conditions
    print('-----------------------------------------\n')
    ...
    data = Dataloader(path=opt['data'],
                      kw_channel=opt['kw_channel'], kw_time=opt['kw_time'], kw_conditions='',
                      norm_data=opt['norm_data'], std_data=opt['std_data'], diff_data=opt['diff_data'],)
    dataset = data.get_data()

    df_labels = pd.read_csv(opt['data'])
    # Recuperiamo la colonna 'Condition' (o quella specificata)
    # Assicurati che il nome della colonna sia corretto
    all_labels = df_labels[default_args['kw_conditions']].values
    
    # Split data function
    def split_data_with_labels(dataset, labels, train_size=.8):
        num_samples = dataset.shape[0]
        shuffle_index = np.arange(num_samples)
        np.random.shuffle(shuffle_index)
        cutoff_index = int(num_samples * train_size)
        
        train_ds = dataset[shuffle_index[:cutoff_index]]
        test_ds = dataset[shuffle_index[cutoff_index:]]
        train_lbls = labels[shuffle_index[:cutoff_index]] # Etichette per il train
        
        return test_ds, train_ds, train_lbls
    ...
    test_dataset, train_dataset, train_labels_for_sampler = split_data_with_labels(dataset, all_labels, opt['train_ratio'])
    if default_args['kw_conditions'] != '':
        # USA LE ETICHETTE DI PANDAS, non quelle del dataset EEG
        train_labels = train_labels_for_sampler 
        if len(train_labels_for_sampler.shape) > 1:
            balancing_labels = train_labels_for_sampler[:, 0]
        else:
            balancing_labels = train_labels_for_sampler
        unique_classes, labels_encoded = np.unique(balancing_labels, return_inverse=True)
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=opt['batch_size'], sampler=sampler, pin_memory=True)
        print(f"AE Training BILANCIATO attivato (128 punti). Classi: {unique_classes}")
    else:
        train_dataloader = DataLoader(train_dataset, batch_size=opt['batch_size'], shuffle=True, pin_memory=True)
        print("Training STANDARD (non bilanciato) - Nessuna condizione fornita.")
