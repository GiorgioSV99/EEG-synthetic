...
def default_inputs_training_gan():
    kw_dict = {
        ...
        'hidden_dim': [int, 'Hidden dimension of the GAN components', 128, 'Hidden dimension: '],
...
def default_inputs_generate_samples():
    ...
    return kw_dict
def default_inputs_generate_lae():
    kw_dict = {
        'seed': [int, 'Set seed for reproducibility', 42, 'Manual seed: '],
        'model': [str, 'GAN checkpoint (.pt) with the frozen encoder', '', 'Model: '],
        'checkpoint': [str, 'AE checkpoint for encoder weights', '', 'AE Checkpoint: '], 
        'data': [str, 'Real dataset CSV for latent anchoring', '', 'Real data path: '], # AGGIUNTO
        'epsilon': [float, 'Perturbation intensity (Eq. 7)', 0.05, 'Epsilon: '],          # AGGIUNTO
        'save_name': [str, 'File where to store the samples', '', 'Save name: '],
        'kw_time': [str, 'Keyword for time step', 'Time', 'Time label: '],
        'kw_channel': [str, 'Keyword for channel', 'Electrode', 'Channel label: '],
        'sequence_length': [int, 'Length of generated sample', -1, 'Sequence length: '],
        'num_samples_total': [int, 'Total samples to generate', 1000, 'Total samples: '],
        'num_samples_parallel': [int, 'Samples generated in parallel', 50, 'Batch size: '],
        'conditions': [int, '** Target class (e.g. 0 or 1)', 0, 'Condition: '],
        'patch_size': [int, 'Patch size for Transformer', 16, 'Patch size: '],
    }

    return kw_dict


def default_inputs_lae_training():
    kw_dict = {
        # --- Parametri di Sistema ---
        'ddp': [bool, 'Activate distributed training', False, 'Distributed training is active'],
        'seed': [int, 'Set seed for reproducibility', 42, 'Manual seed: '],
        'n_epochs': [int, 'Number of epochs', 150, 'Number of epochs: '],
        'batch_size': [int, 'Batch size', 128, 'Batch size: '],
        'sample_interval': [int, 'Interval for saving reconstruction plots', 50, 'Sample interval: '],

        # --- Architettura ---
        # latent_dim e hidden_dim devono coincidere per il Latent AE
        # Assicurati che siano divisibili per num_heads (es. 100 / 5 heads = 20)
        'latent_dim': [int, 'Dimension of the latent vector z', 128, 'Latent dimension (z): '],
        'hidden_dim': [int, 'Dimension of Transformer embeddings', 128, 'Hidden dimension: '],
        'num_layers': [int, 'Number of layers of the components', 4, 'Number of layers: '],
        'patch_size': [int, 'Patch size of the divided sequence', 16, 'Patch size: '],
        'num_heads': [int, 'Number of attention heads', 8, 'Number of heads: '],

        # --- Ottimizzazione ---
        'ae_lr': [float, 'Learning rate for the Autoencoder', 0.0001, 'AE learning rate: '],
        # Manteniamo questi per compatibilità interna alla repository
        #'discriminator_lr': [float, 'Unused in AE', 0.0001, 'Disc LR: '],
        #'generator_lr': [float, 'Unused in AE', 0.0001, 'Gen LR: '],

        # --- Gestione Dati e Percorsi ---
        'data': [str, 'Path to the dataset', os.path.join('data', 'tuo_eeg.csv'), 'Dataset: '],
        'checkpoint': [str, 'Path to a pre-trained Latent AE', '', 'Using pre-trained AE: '],
        'autoencoder': [str, 'Path to the TEMPORAL compression AE', '', 'Using temporal AE: '],
        'save_name': [str, 'Name to save latent AE model', 'latent_ae_v1', 'Model save name: '],

        # --- Metadati Dataset (Sincronizzati con Dataloader originale) ---
        # Fondamentale: kw_conditions come LISTA
        'kw_conditions': [str, '** Specific numeric conditions', '', 'Conditions: '],
        'kw_time': [str, 'Keyword for time steps', 'Time', 'Time label: '],
        'kw_channel': [str, 'Keyword for channels', 'Channel', 'Channel label: '],

        # n_conditions deve essere il numero effettivo di classi (es. 4)
        'n_conditions': [int, 'Number of classes/conditions', 1, 'Num conditions: '],
        'n_channels': [int, 'Number of EEG channels', 8, 'Num channels: '],

        # sequence_length del segnale EEG (esclusi i punti delle label)
        'sequence_length': [int, 'Total sequence length', 150, 'Sequence length: '],
        # --- Logica Specifica ---
        'class_balanced': [bool, 'Activate class-balanced sampling', True, 'Class balancing: '],
        'loss_type': [str, 'Loss function to use', 'L2', 'Loss type: '],
        'condition_strategy': [str, 'Conditioning (e.g. first_point)', 'first_point', 'Strategy: '],
    }
...
def parse_arguments(arguments, kw_dict=None, file=None):
    ...
    if file is not None:
        ...
        elif file == 'lae_training_main.py':
            system_args = default_inputs_lae_training() # Chiama la tua funzione!
            helper = HelperMain(system_args)
        else:
